...
class _MiCvScreenState extends State<MiCvScreen> {
  ...
  void _loadCandidatoProfile() {
    ...
    if (userId != null) {
      setState(() {
        _candidatoFuture = _airtableService.getCandidatoProfile(userId);
      });
    }
  }

  Future<void> _updateAirtableField(String recordId, Map<String, dynamic> fields) async {
    final success = await _airtableService.updateCandidatoProfile(recordId, fields);
    if (mounted) {
      if (success) {
        _loadCandidatoProfile(); // Recarga el perfil para mostrar los cambios
      } else {
  ...
  Future<void> _changePdf(Candidato candidato) async {
    ...
    try {
      // Borra el archivo antiguo si existe una URL válida
  ...
  Future<void> _deleteProfile(Candidato candidato) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('¿Estás seguro de que quieres eliminar tu perfil de candidato? Esta acción no se puede deshacer.'),
        actions: [
    ...
    try {
      ...
      // Borrar de Airtable
      final success = await _airtableService.deleteCandidatoProfile(candidato.recordId);
  ...
  Widget build(BuildContext context) {
    // ✅ CAMBIO: Se elimina el Scaffold, ya que el dashboard principal lo provee.
