  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Column(
        ...
        children: [
          // --- 👇 CAMBIO CLAVE AQUÍ ---
          // Ahora usamos 'previewUrl' que viene del thumbnail del PDF.
          if (formato.previewUrl != null)
            Image.network(
              formato.previewUrl!, // <-- Se usa 'previewUrl' en lugar de 'imageUrl'
              height: 150,
              ...
              errorBuilder: (context, error, stackTrace) => Container(height: 150, color: Colors.grey[300], child: const Center(child: Icon(Icons.broken_image, color: Colors.white))),
            )
          else
            Container(height: 150, color: Colors.grey[300], child: const Center(child: Icon(Icons.image_not_supported, size: 50, color: Colors.grey))),
          // --- FIN DEL CAMBIO ---
          Padding(
...
// --- Widget que muestra la lista de formatos (Sin cambios) ---
    ...
    return FutureBuilder<List<Formato>>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) => FormatoCard(formato: formatos[index]),
        );
      },
    );
  }
...
class _FormatosScreenState extends State<FormatosScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Formatos'), backgroundColor: Colors.deepPurple),
